...
class DebuggingDirectivesSpec extends RoutingSpec {
  ...
  "The 'logRequest' directive" should {
    "produce a proper log message for incoming requests" in {
      ...
      Get("/hello") ~> route ~> check {
        ...
        debugMsg shouldEqual "1: HttpRequest(HttpMethod(GET),http://example.com/hello,List(),HttpEntity.Strict(none/none,ByteString.empty,HttpProtocol(HTTP/1.1))\n"
      }
    }
  }
  ...
  "The 'logResponse' directive" should {
    "produce a proper log message for outgoing responses" in {
      ...
      Get("/hello") ~> route ~> check {
        response shouldEqual Ok
        debugMsg shouldEqual "2: Complete(HttpResponse(200 OK,List(),HttpEntity.Strict(none/none,ByteString.empty,HttpProtocol(HTTP/1.1)))\n"
      }
    }
  }
    ...
    "produce proper log messages for outgoing responses, thereby showing the corresponding request" in {
      ...
      Get("/hello") ~> route ~> check {
        ...
        debugMsg shouldEqual """|3: Response for
                              |  Request : HttpRequest(HttpMethod(GET),http://example.com/hello,List(),HttpEntity.Strict(none/none,ByteString.empty,HttpProtocol(HTTP/1.1))
                              |  Response: Complete(HttpResponse(200 OK,List(),HttpEntity.Strict(none/none,ByteString.empty,HttpProtocol(HTTP/1.1)))
